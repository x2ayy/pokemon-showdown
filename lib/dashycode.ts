...
function streamPeek(stream: DashyStream, readLength: number, readMask = 0xffff >> (16 - readLength)) {
...
function streamRead(stream: DashyStream, readLength: number, readMask = 0xffff >> (16 - readLength)) {
...
export function encode(str: string, allowCaps = false) {
  ...
  for (let i = 0; i < str.length + 1; i++) {
    ...
    if (
      capBuffer &&
      (!isLowercase || alphaIndex >= 8 || i === str.length)
    ) {
      // flush cap buffer
      if (capBuffer === 0xd) {
    ...
    if (
      ...
      curCharCode === 32 &&
      // space
      ((97 <= str.charCodeAt(i + 1) && str.charCodeAt(i + 1) <= 122) ||
        // a-z
        (65 <= str.charCodeAt(i + 1) && str.charCodeAt(i + 1) <= 90) ||
        // A-Z
        (48 <= str.charCodeAt(i + 1) && str.charCodeAt(i + 1) <= 57))
      // 0-9
    ) {
      safePart += "-";
      streamWrite(unsafeStream, 2, 0x0);
    ...
    if (curCharCode === -1) {
      ...
      (unsafeMapIndex = UNSAFE_MAP.indexOf(str.charAt(i))) >= 0
    ) {
    ...
    if (curChar !== "-") {
      ...
      do {
        switch (streamRead(unsafeStream, 2, 0x3)) {
          ...
          case 0x2:
            curChar = UNSAFE_MAP.charAt(
              streamRead(unsafeStream, 5, 0x1f)
            );
            isEmpty = false;
            break;
          case 0x3:
            if (streamRead(unsafeStream, 1, 0x1)) {
              curChar = String.fromCharCode(
                streamRead(unsafeStream, 16, 0xffff)
              );
            } else {
              curChar = " ";
            }
